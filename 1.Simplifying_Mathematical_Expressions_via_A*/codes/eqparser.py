...
class Node:
    ...
    def __str__(self):
        ...
        if (self.type == "EQUALS") or (self.type == "BINARYOP"):
            if self.leaf is '^': 
                if(self.children[0].type == 'BINARYOP' or self.children[0].type == 'UNARYOP'):
                    str0 = "(" + str(self.children[0]) + ")"
                else:
                    str0 = str(self.children[0])
                if(self.children[1].type == 'BINARYOP' or self.children[1].type == 'UNARYOP'):
                    str1 = "(" + str(self.children[1]) + ")"
                else:
                    str1 = str(self.children[1])
                return str0 + pretty_printing_colors[self.type] + self.leaf + bcolors.ENDC + str1
            elif( self.leaf == '*' or self.leaf == '/'):
                if(self.children[1].type == 'BINARYOP' and (self.children[1].leaf == '+' or self.children[1].leaf == '-') ):
                    str1 = "(" + str(self.children[1]) + ")"
                elif(self.leaf == '/' and self.children[1].leaf == '*'):
                    str1 = "(" + str(self.children[1]) + ")" 
                else:
                    str1 = str(self.children[1])
                if(self.children[0].type == 'BINARYOP' and (self.children[0].leaf == '+' or self.children[0].leaf == '-') ):
                    str0 = "(" + str(self.children[0]) + ")"
                else:
                    str0 = str(self.children[0])
                return str0 + pretty_printing_colors[self.type] + " " + self.leaf + " " + bcolors.ENDC + str1
            elif(self.leaf == '-'):
                if(self.children[1].leaf == '+'):
                    str1 = "(" + str(self.children[1]) + ")"
                else:
                    str1 = str(self.children[1])
                return str(self.children[0]) + pretty_printing_colors[self.type] + " " + self.leaf + " " + bcolors.ENDC + str1
            else:
        ...
        if (self.type == "UNARYOP"):
            if(self.children.type == 'BINARYOP' and self.children.leaf != '^'):
                str0 = "("+str(self.children)+")"
            else:
                str0 = str(self.children)
            return  pretty_printing_colors[self.type] + self.leaf + bcolors.ENDC + str0
